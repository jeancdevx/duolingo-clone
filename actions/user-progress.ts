...
import { MAX_HEARTS, MIN_HEARTS, POINTS_TO_REFILL_HEARTS } from '@/constants'
...
export const reduceHearts = async (challengeId: number) => {
  ...
  if (currentUserProgress.hearts === MIN_HEARTS) return { error: 'hearts' }

  await db
    .update(userProgress)
    .set({
      hearts: Math.max(currentUserProgress.hearts - 1, MIN_HEARTS)
    })
    .where(eq(userProgress.userId, userId))
